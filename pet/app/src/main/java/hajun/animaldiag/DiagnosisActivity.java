...
public class DiagnosisActivity extends ListActivity {
	...
	Handler m_handler = new Handler(){
		...
		public void handleMessage(Message msg){
			...
			for(int i = 0; i < m_share.m_vecSelected.size(); i++)
				System.out.println(m_share.m_vecSelected.get(i).strName); // 선택된 증상 출력
		}
	};
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		m_share.m_vecSelected.clear(); // Vector 초기화
	...
	public void SetButtons()
	{
		...
		btnDiagnosis.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				double dPattern[] = new double[m_share.m_nSymptom]; //dPattern 배열 객체 생성
				for(int i = 0; i < m_share.m_nSymptom; i++)
					dPattern[i] = 0; // 0으로 초기화

				for(int i = 0; i < m_share.m_vecSelected.size(); i++)
					dPattern[m_share.m_vecSelected.get(i).nNum-1] = 1; // 선택된 증상이면 1, 아니면 0으로 하는 for문 인듯

				m_share.SetSelectedSymptom(dPattern, true);

				Intent intent = new Intent(DiagnosisActivity.this, ResultActivity.class); // intent 액티비티간 화면전환 (DiagnosisActivity -> ResultActivity)
